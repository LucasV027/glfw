...
void glProgram::load(const std::filesystem::path &vertexShader, const std::filesystem::path &fragmentShader) {
    vsPath = vertexShader;
...
void glProgram::loadShader(const std::filesystem::path &path, const GLenum shaderType) const {
...
[[nodiscard]] GLuint glProgram::get() const {
    return shaderProgramme;
}


std::string glProgram::readFile(const std::filesystem::path &filePath) {
